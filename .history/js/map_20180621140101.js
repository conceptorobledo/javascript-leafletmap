...
function homesData(data) {
    ...
    Object.keys(homes).map(key => {
        ...
        let theMarker;
        if (estado == "actividad") {
            theMarker = statusOkMarker;
        }
        else if (estado == "inactividad") {
            theMarker = defaultMarker;
        }
        else {
            console.log('no esta definido');
            return;
        }
        marker[key] = L.marker([lat, lng], { icon: theMarker, homeId: key, address: address });
...
async function onClick(e) {
    ...
    await homesRef.child(homeId + '/patrols').orderByKey().limitToLast(10).once('value', patrolPerHomeReport, errData);
